...
export class SqlServerBridgeClient extends EventEmitter {
  ...
  private getBridgeExecutablePath(): string {
    ...
    const searchPaths = [
      ...
      path.join(bridgeDir, "bin", "Release", "net8.0", runtimeId, dllName),
      // Debug, runtime-specific
      path.join(bridgeDir, "bin", "Debug", "net8.0", runtimeId, executableName),
      path.join(bridgeDir, "bin", "Debug", "net8.0", runtimeId, dllName),
      // Release, framework-dependent
      path.join(bridgeDir, "bin", "Release", "net8.0", executableName),
      path.join(bridgeDir, "bin", "Release", "net8.0", dllName),
      // Debug, framework-dependent
      path.join(bridgeDir, "bin", "Debug", "net8.0", executableName),
      path.join(bridgeDir, "bin", "Debug", "net8.0", dllName),
    ];
